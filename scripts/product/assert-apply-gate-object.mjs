...
const requiredFiles = [
  ...
  "configs/runtime/apply_gate_policy.json",
...
const schemaPath = "configs/contracts/apply_gate_object.schema.json";
const policyPath = "configs/runtime/apply_gate_policy.json";
...
if (schema?.properties?.object_type?.const !== "ADJUTORIX_APPLY_GATE_OBJECT") {
...
if (policy?.policy_type !== "ADJUTORIX_APPLY_GATE_POLICY") {
...
for (const gate of [
  ...
  "apply_gate_may_not_mutate_files",
  "apply_gate_may_not_apply_patch",
  "apply_gate_unlocks_apply_only_on_verify_receipt_pass",
  "apply_execution_requires_operator_trigger",
  "apply_execution_requires_apply_receipt",
...
for (const phrase of [
  ...
  "createApplyGateObject",
  "validateApplyGateObject",
  "ADJUTORIX_APPLY_GATE_OBJECT",
  "apply.gate.created",
  "apply_gate_receipt",
  "Apply Gate object",
  "Create apply gate",
  "applyGateObjectReady",
  "may_mutate_files: false",
  "may_apply_patch: false",
  "apply_unlocked: true"
]) {
  if (!cockpit.includes(phrase)) failures.push({ code: "COCKPIT_MISSING_APPLY_GATE_PHRASE", phrase });
}

const report = {
  product: "ADJUTORIX_VERIFY_RECEIPT_TO_APPLY_GATE",
...
fs.writeFileSync("reports/current/apply-gate-object-readiness.json", JSON.stringify(report, null, 2) + "\n");

const historyDir = `reports/history/apply-gate-object-${report.timestamp.replace(/[:.]/g, "-")}`;
fs.mkdirSync(historyDir, { recursive: true });
fs.writeFileSync(`${historyDir}/apply-gate-object-readiness.json`, JSON.stringify(report, null, 2) + "\n");
...
console.log(`ADJUTORIX_APPLY_GATE_OBJECT_READINESS=${report.verdict}`);
console.log("REPORT=reports/current/apply-gate-object-readiness.json");
