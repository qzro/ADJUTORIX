...
const COMMANDS: Record<FeatureKey, Array<{ label: string; command: string }>> = {
  agent: [
    { label: "Workspace map", command: "find . -maxdepth 3 -type f | sed 's#^./##' | head -160" },
    { label: "Repo status", command: "git status --short" },
    { label: "Recent commits", command: "git log --oneline -12" },
  ],
  plan: [
    { label: "Create plan", command: "find .adjutorix/objects -type f 2>/dev/null | sort | tail -40" },
    { label: "Plan objects", command: "find .adjutorix/objects -type f 2>/dev/null | sort | tail -120" },
    { label: "Latest plan", command: "latest=$(find .adjutorix/objects -type f 2>/dev/null | sort | tail -1); test -n \"$latest\" && cat \"$latest\" || true" },
  ],
  code: [
    { label: "Search TODO", command: "grep -RIn \"TODO\\|FIXME\\|throw new Error\\|console.error\" app packages src scripts 2>/dev/null | head -120" },
    { label: "Large files", command: "find . -type f -not -path './.git/*' -not -path './node_modules/*' -not -path './dist/*' -size +100k | head -80" },
    { label: "Routes", command: "find . -type f \\( -name '*.js' -o -name '*.ts' -o -name '*.tsx' \\) -not -path './node_modules/*' -print | xargs grep -n \"route\\|router\\|endpoint\\|api\" 2>/dev/null | head -140" },
  ],
  diff: [
    { label: "Diff stat", command: "git diff --stat" },
    { label: "Diff body", command: "git diff | head -260" },
    { label: "Staged diff", command: "git diff --cached --stat && git diff --cached | head -220" },
  ],
  verify: [
    { label: "Typecheck", command: "pnpm --filter @adjutorix/app run build:ts" },
    { label: "Target tests", command: "pnpm --filter @adjutorix/app exec vitest run tests/renderer/operator_kernel_live_surface_contract.test.ts tests/renderer/operator_surface_spine_contract.test.ts tests/renderer/operator_unified_control_spine_contract.test.ts" },
    { label: "Full verify", command: "pnpm run verify" },
  ],
  patch: [
    { label: "Patch preview", command: "git diff --stat && git diff | head -260" },
    { label: "Changed files", command: "git status --short" },
    { label: "Patch custody", command: "find .adjutorix reports/current -type f 2>/dev/null | sort | tail -120" },
  ],
  ledger: [
    { label: "Ledger files", command: "find .adjutorix reports/current -type f 2>/dev/null | sort | tail -180" },
    { label: "Report heads", command: "find reports/current -maxdepth 1 -type f 2>/dev/null | sort | tail -80" },
    { label: "Receipt hashes", command: "find .adjutorix reports/current -type f 2>/dev/null | xargs shasum 2>/dev/null | tail -80" },
  ],
  diagnostics: [
    { label: "Runtime logs", command: "find . -name '*.log' -type f -not -path './node_modules/*' 2>/dev/null | head -80" },
    { label: "Electron app files", command: "find /Applications/Adjutorix.app/Contents/Resources -maxdepth 6 -type f 2>/dev/null | head -160" },
    { label: "Package config", command: "cat packages/adjutorix-app/package.json | head -220" },
  ],
  kernel: [
    { label: "Kernel bridge", command: "grep -RIn \"operatorKernel\\|operatorKernelReceiptId\\|operatorKernelHash\\|previousKernelHash\" packages/adjutorix-app/src configs tests | head -180" },
    { label: "IPC kernel", command: "grep -RIn \"adjutorix:operatorKernel\\|createReceipt\\|lastHash\" packages/adjutorix-app/src | head -120" },
    { label: "Kernel policy", command: "cat configs/runtime/operator_kernel_live_surface_policy.json 2>/dev/null || true" },
  ],
  package: [
...
function between(value: string, start: string, end: string): string {
  ...
  if (a >= 0 && b > a) return value.slice(a + start.length, b).replace(/^\s*\n/, "").replace(/\n\s*$/, "");
  return value;
}
...
function isFileLine(path: string): boolean {
  ...
  if (path.startsWith("$")) return false;
  if (path.includes('"path"') || path.includes('"files"') || path.includes('":"')) return false;
  if (path.length > 220) return false;
  return path.includes("/") || path.includes(".") || /^[A-Z0-9_.-]{3,}$/i.test(path);
}
  ...
  return Array.from(
    new Set(
      intent
        ...
        .filter((word) => !["the", "and", "for", "with", "what", "are", "you", "this", "that", "change", "make", "add", "fix"].includes(word)),
    ),
  ).slice(0, 8);
}
...
export function AdjutorixPowerWorkbench(): JSX.Element {
  ...
  const [terminal, setTerminal] = useState<string[]>(["ADJUTORIX LIVE. Open a folder, ask, inspect, verify, patch."]);
  const [chat, setChat] = useState<ChatMessage[]>([
    { id: id(), role: "adjutorix", body: "Alive. Open a workspace or ask me what to change. I will create a governed plan and show the real command/output boundary." },
  ]);
  ...
  const visibleFiles = useMemo(() => {
    ...
    const list = needle ? files.filter((file) => file.path.toLowerCase().includes(needle)) : files;
  ...
  const invokeCommand = useCallback(
    async (nextCommand: string, cwd = workspace, show = true): Promise<unknown> => {
      if (!api?.runCommand) {
      ...
      try {
        const result = await api.runCommand({ command: trimmed, cwd: cwd || undefined, workspace: cwd || undefined });
  ...
  const runFeatureCommand = useCallback(
    async (label: string, nextCommand: string) => {
      ...
      if (label.toLowerCase().includes("verify") || nextCommand.includes("verify")) {
...
printf '\\n__ADJUTORIX_FILE_END__\\n'`;
      ...
      const result = await invokeCommand(cmd, cwd, false);
      const content = between(stdout(result), "__ADJUTORIX_FILE_BEGIN__", "__ADJUTORIX_FILE_END__");
  ...
  const createPlan = useCallback(async () => {
    ...
    if (!intent) {
      addChat("adjutorix", "Alive. Give me a concrete target: file, behavior, bug, or feature.");
      return;
    }
    ...
    if (/alive|there|working|online/i.test(intent)) {
      addChat("adjutorix", `Yes. I am live. Workspace: ${workspace || "not loaded"}. Files indexed: ${files.length}. Apply is ${verifyReady ? "available after review" : "blocked until verification"}.`);
      return;
    }

    if (!workspace) {
      addChat("adjutorix", "Open a workspace first. Then I can inspect files, search, plan, diff, and verify.");
      return;
    }
...
PY`;
  ...
  const writeActiveFile = useCallback(async () => {
    ...
    if (!verifyReady) {
      addChat("adjutorix", "Apply is blocked. Run Verify first, inspect Diff, then apply.");
      return;
    }
...
PY`;
    ...
    const result = await invokeCommand(cmd, workspace, false);
    addChat("adjutorix", `Applied buffer after verify gate.\n${display(result)}`);
  ...
  const featureTabs: FeatureKey[] = ["agent", "plan", "code", "diff", "verify", "patch", "ledger", "diagnostics", "kernel", "package", "ipc"];

  return (
    <section className="adjutorix-real-agent" data-busy={busy ? "true" : "false"}>
      ...
      <aside className="adjutorix-real-explorer">
        ...
        <section className="adjutorix-real-workspace">
          ...
          <div>
            <input value={pathInput} onChange={(event) => setPathInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void scanWorkspace(pathInput); }} />
            <button type="button" onClick={() => void scanWorkspace(pathInput)}>Load</button>
          </div>
        </section>

        <input className="adjutorix-real-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search real files..." />

        <div className="adjutorix-real-filelist">
          {visibleFiles.length ? visibleFiles.map((file) => (
            <button key={file.path} className={file.path === activePath ? "is-active" : ""} type="button" onClick={() => void readFile(file)}>
            ...
            <article className="adjutorix-real-start">
              ...
              <button type="button" onClick={() => void scanWorkspace(pathInput || workspace)}>Scan</button>
            </article>
          )}
        </div>
      </aside>
      ...
      <main className="adjutorix-real-main">
        <header className="adjutorix-real-topbar">
          ...
          <input value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void createPlan(); }} placeholder="Ask Adjutorix what to change..." />
          <button type="button" onClick={() => void createPlan()}>Run agent</button>
          <button type="button" onClick={() => void runFeatureCommand("Verify", "pnpm run verify")}>Verify</button>
          <button type="button" onClick={() => void runFeatureCommand("Diff", "git diff --stat && git diff | head -260")}>Diff</button>
        </header>

        <nav className="adjutorix-real-tabs">
          {tabs.map((tab) => (
            <button key={tab.path} className={tab.path === activePath ? "is-active" : ""} type="button" onClick={() => setActivePath(tab.path)}>
        ...
        <section className="adjutorix-real-editor">
          ...
          <textarea spellCheck={false} value={activeTab?.content ?? ""} onChange={(event) => updateBuffer(event.target.value)} />
        </section>

        <section className="adjutorix-real-terminal">
          <header>
            <strong>Terminal</strong>
            <input value={command} onChange={(event) => setCommand(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void runFeatureCommand("Terminal", command); }} />
          </header>
          <div>{terminal.map((line, index) => <pre key={index}>{line}</pre>)}</div>
        </section>
      </main>
      ...
      <aside className="adjutorix-real-side">
        <nav>
          {featureTabs.map((tab) => (
            <button key={tab} className={feature === tab ? "is-active" : ""} type="button" onClick={() => setFeature(tab)}>{tab}</button>
          ))}
        </nav>
        ...
        <section className="adjutorix-real-chat">
          ...
          <textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Example: add barcode validation, refactor scanner flow, update tests..." />
          <button type="button" onClick={() => void createPlan()}>Generate real plan</button>
        ...
        <section className="adjutorix-real-actions">
          ...
          <div>
            {COMMANDS[feature].map((action) => (
              <button key={action.label} type="button" onClick={() => void runFeatureCommand(action.label, action.command)}>{action.label}</button>
            ))}
            {feature === "agent" && <button type="button" onClick={() => void runBridgeFeature("Runtime snapshot", "runtime.snapshot", "node -e \"console.log(JSON.stringify(process.versions,null,2))\"")}>Runtime bridge</button>}
            {feature === "kernel" && <button type="button" onClick={() => setFeatureOutput(asJson(adjutorixOperatorKernelBridge() ?? "No operator kernel bridge"))}>Kernel object</button>}
            {feature === "patch" && <button type="button" onClick={() => void saveDraft()}>Save draft</button>}
            {feature === "patch" && <button type="button" onClick={() => void writeActiveFile()}>Apply active buffer</button>}
          </div>
        </section>
        ...
        <section className="adjutorix-real-gate">
          <article data-ok={workspace ? "true" : "false"}><strong>Workspace</strong><span>{workspace ? "ready" : "missing"}</span></article>
          <article data-ok={files.length ? "true" : "false"}><strong>Index</strong><span>{files.length} files</span></article>
          <article data-ok={verifyReady ? "true" : "false"}><strong>Verify gate</strong><span>{verifyReady ? "open" : "required"}</span></article>
          <article data-ok={verifyReady ? "true" : "false"}><strong>Apply gate</strong><span>{verifyReady ? "review then apply" : "blocked"}</span></article>
        </section>
      </aside>
