 ...
 * Wrapped in try/catch so extension host crash-loop leaves logs (Output + console).
 */
export async function activate(context: vscode.ExtensionContext) {
  ...
  try {
    ...
    if (!workspaceRoot) {
      ...
      out.appendLine("→ For /check, /fix, etc. to run at repo root, open the repo root folder (e.g. ADJUTORIX/ADJUTORIX), not a subpackage.");
    ...
    if (workspaceRoot) {
      out.appendLine(`[extension] agent workspaceRoot: ${workspaceRoot}`);
    ...
    out.appendLine("[activate] before register view");
    ...
    out.appendLine("[activate] ok");
    vscode.window.showInformationMessage("Adjutorix activated.");
  } catch (e) {
    out.appendLine("[activate] FAILED: " + String(e));
